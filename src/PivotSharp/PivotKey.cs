...
namespace PivotSharp
{
	...
	public class PivotKey
	{
		...
		public string FlattenedRowKey {
			...
			set { RowKeys = value.Split(new [] {','}, StringSplitOptions.RemoveEmptyEntries).ToList();  }
		}

		public string FlattenedColKey {
			get { return string.Join(",", ColKeys); }
			set { ColKeys = value.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).ToList(); }
		}

		public override string ToString() {
			return string.Format("{0}|{1}", FlattenedRowKey, FlattenedColKey);
		}
		...
		public static bool operator == (PivotKey key1, PivotKey key2) {
